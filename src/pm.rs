...
use crate::{
    network::{get_arch_name, TopicManifest, TopicManifests},
};
...
use crate::pk::{refresh_cache, find_stable_version_of, get_updated_packages};
...
pub fn switch_topics(
    ...
    let mut updated = updated.into_iter().map(|x| x.package_id).collect::<Vec<_>>();
